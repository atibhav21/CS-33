 ...
 */
int bitParity(int x) {
       //algorithm: Two numbers contain odd number of 0s if x ^ y contains odd number of 0s. 
...
int rotateRight(int x, int n) {
  ...
  int rotatedDigits = (x>> n) & ~( (~0) << (32 + (~n + 1))); // & with ~ (-1 << 32)
  int lx = x << (32 + (~n + 1)); //left shifted by 32 - n
...
int byteSwap(int x, int n, int m) {
  ...
  return (x | (m_n | m_m));   //form a single number with m_n and m_m swaped
...
int fitsShort(int x) {
  ...
  x = x >> 15; 
  // if x can fit in short, x is now all 0s or all 1s.
  // performing a right shift would lead to the same number, if x can fit in short
